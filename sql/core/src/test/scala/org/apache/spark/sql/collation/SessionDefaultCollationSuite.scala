...
class SessionDefaultCollationSuite extends QueryTest with SharedSparkSession {
  ...
  private val prefix = s"${CollationFactory.CATALOG}.${CollationFactory.SCHEMA}"
  private val testTableName = "test_table"

  override protected def beforeEach(): Unit = {
    super.beforeEach()
    spark.conf.set(SQLConf.SESSION_LEVEL_COLLATIONS_ENABLED.key, "true")
    // Disable single pass analyzer for this suite, since session collation resolution is
    // currently not supported in single pass.
    spark.conf.set(SQLConf.ANALYZER_DUAL_RUN_LEGACY_AND_SINGLE_PASS_RESOLVER.key, "false")
  }
  ...
  test("SELECT with WHERE clause comparing literals applies session collation") {
    withSessionCollation {
      ...
      withTable(testTableName) {
        sql(s"CREATE TABLE $testTableName (id INT)")
