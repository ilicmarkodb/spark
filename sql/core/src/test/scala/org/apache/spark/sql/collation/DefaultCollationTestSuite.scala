...
abstract class DefaultCollationTestSuiteV1 extends DefaultCollationTestSuite {
  ...
  testString("CREATE VIEW pins UTF8_BINARY when the schema has no default collation") { _ =>
    // CREATE VIEW resolves the default collation at create time. If the schema has no
    // default, the view falls back to `UTF8_BINARY` rather than persisting `None`, so a
    // later `ALTER SCHEMA ... DEFAULT COLLATION` cannot retroactively change how the view's
    // body is re-analyzed. ALTER VIEW AS inherits the pinned default, too.
    withDatabase(testSchema) {
